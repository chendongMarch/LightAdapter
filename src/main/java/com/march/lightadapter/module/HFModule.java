...
public class HFModule extends AbstractModule {
    ...
    public <D> ViewHolder<D> getHFViewHolder(int viewType) {
        ...
        if (isFooter) {
            mFooterHolder = holder = new ViewHolder<D>(mAttachAdapter.getContext(), footerView, viewType);
        } else if (isHeader) {
            mHeaderHolder = holder = new ViewHolder<D>(mAttachAdapter.getContext(), headerView, viewType);
        }
